...
class OrderModelViewSet(ModelViewSet):
    ...
    def monthly_income(self, request, year=None):
        ...
        for i in bruhs:
            ...
            rented_months_target_year = (end_date.year - start_date.year) * 12 + end_date.month - start_date.month + 1  # Adjusted calculation
            paid_months = i.paid_payment // i.rent_price
            asd=i.paid_payment % i.rent_price
            months_before_target_year = (start_date.year - i.start_date.year) * 12 + start_date.month - i.start_date.month
    ...
    def yearly_income(self, request, year=None):
        ...
        orders1=OrderModel.objects.filter(start_date__year__lte=target_year, end_date__year__gte=target_year,olast_action="deleted")
        for i in orders:
            if i.start_date.year==target_year :
                rented_months_target_year=min(13-i.start_date.month,i.monthly_payment())
                paid_months=i.paid_payment//i.rent_price
                paid_months_target_year=min(paid_months,rented_months_target_year) if paid_months>=0 else 0
            else:
                paid_months=i.paid_payment//i.rent_price
                months_before_target_year=(target_year-i.start_date.year)*12+1-i.start_date.month
                ...
                paid_months_target_year=min(h,rented_months_target_year) if h>=0 else 0
...
class OutlayModelViewSet(ModelViewSet):
    ...
    def monthly_expenses(self, request, year=None):
        result=[0]*12
        target_year=year if year!=None else datetime.datetime.now().year
        outlays=OutlayModel.objects.filter(created_date__year=target_year).exclude(elast_action="deleted")
        for i in outlays:
    ...
    def perform_destroy(self, instance):
        if instance.object_model=="user_model":
            if instance.action_type=="created":
                ...
                user=get_object_or_404(UserModel,id=instance.object_id)
                shadow=get_object_or_404(ShadowUserModel,id=instance.shadow_object_id)
            ...
            if instance.action_type == "created":
                ...
                banner = get_object_or_404(BannerModel,id=instance.object_id)
                shadow=get_object_or_404(ShadowBannerModel,id=instance.shadow_object_id)
            ...
            if instance.action_type == "created":
                ...
                order = get_object_or_404(OrderModel,id=instance.object_id)
                shadow=get_object_or_404(ShadowOrderModel,id=instance.shadow_object_id)
            ...
            if instance.action_type == "created":
                ...
                payment = get_object_or_404(PaymentModel,id=instance.object_id)
                shadow=get_object_or_404(ShadowPaymentModel,id=instance.shadow_object_id)
                ...
                if sclient.full_payment-sclient.paid_payment<shadow.payment_amount:
                    ...
                    y=sclient.paid_payment+shadow.payment_amount-payment.payment_amount
            ...
            if instance.action_type=="created":
                ...
                outlay=get_object_or_404(OutlayModel,id=instance.object_id)
                shadow=get_object_or_404(ShadowOutlayModel,id=instance.shadow_object_id)
